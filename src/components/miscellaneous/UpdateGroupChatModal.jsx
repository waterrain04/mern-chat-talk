...
import { Box, FormControl, IconButton, Spinner, useDisclosure } from '@chakra-ui/react'
import {
  Modal,
  ModalOverlay,
  ModalContent,
...
const UpdateGroupChatModal = ({fetchAgain,fetchMessages,setFetchAgain}) => {
  ...
  const handleDelete =async (myUser)=>{
    ...
    if(selectedChat.groupAdmin._id !== user._id && myUser._id!==user._id){
      toast({
    ...
    try{
        ...
        toast({
          ...
          description: `${myUser.name} has been removed`,
          status:"success",
          duration: 5000,
          isClosable:true,
          position:"bottom"
        ...
        myUser._id === user._id ? setSelectedChat() :setSelectedChat(data);
  ...
  const handleAddUser = async(myUser)=>{
    if(selectedChat.users.find(u=>u._id===myUser._id)){
      toast({
